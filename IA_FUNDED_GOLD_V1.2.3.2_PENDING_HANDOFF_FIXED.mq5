...
#property description "IA FUNDED GOLD V1.2.3.2 - handoff transacional entre pendente e entrada a mercado."
...
bool CanAddTowardBiasV122(ENUM_POSITION_TYPE direction);
bool CanAddCounterBiasRecoveryV122(ENUM_POSITION_TYPE direction);
...
bool RequestPendingCancelV1231(bool buy_side, ulong ticket, string reason);
...
bool IsGeometryApplicableV121(ENUM_POSITION_TYPE direction, ENUM_ENTRY_GEOMETRY_V121 geometry, int regime, string &reason);
...
bool FreeSlotForPriorityRebalanceV122(ENUM_POSITION_TYPE direction)
{
   ...
   for(int pass = 0; pass < 2; pass++)
   {
      for(int i = OrdersTotal() - 1; i >= 0; i--)
      {
         ...
         if(RequestPendingCancelV1231(buy_pending, ticket, "liberar_slot_rebalance"))
         {
...
bool FreePendingSlotForMarketEntryV123(EntryCandidateV121 &candidate, string &reason)
{
   ...
   for(int pass = 0; pass < 2; pass++)
   {
      for(int i = OrdersTotal() - 1; i >= 0; i--)
      {
         ...
         bool buy_pending = IsBuyPendingType(type);
         if(RequestPendingCancelV1231(buy_pending, ticket, "liberar_slot_entrada_mercado"))
         {
...
bool RequestPendingCancelV1231(bool buy_side, ulong ticket, string reason)
{
   ...
   bool emergency_cancel = (reason == "cancelar_todas_pendentes");
...
bool IsGeometryApplicableV121(ENUM_POSITION_TYPE direction, ENUM_ENTRY_GEOMETRY_V121 geometry, int regime, string &reason)
{
   ...
   if(geometry == ENTRY_GEOMETRY_MOMENTUM && !neutral_momentum_v123 && !CanAddTowardBiasV122(direction))
   {
      reason = "bias_bloqueia_momentum";
      return false;
   }

   if(geometry == ENTRY_GEOMETRY_RECOVERY && !CanAddCounterBiasRecoveryV122(direction))
   {
...
bool GetDesiredPendingOrderV121(bool buy_side, ENUM_ORDER_TYPE &order_type, double &price, ENUM_ENTRY_GEOMETRY_V121 &geometry, string &reason, ulong exclude_order_ticket)
{
   ...
   if(!IsGeometryApplicableV121(direction, geometry, regime, reason))
      return false;
   ...
   if(MathAbs(regime) == 2 && geometry == ENTRY_GEOMETRY_RECOVERY)
   {
      bool counter_extreme = ((direction == POSITION_TYPE_BUY && regime < 0) ||
                              (direction == POSITION_TYPE_SELL && regime > 0));
      if(counter_extreme && CountRecoveryContraExtremaV121(direction) >= MaxEntradasRecoveryContraExtremaV121)
      {
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(!GetDesiredPendingOrderCurrentMode(buy_side, desired_type, desired_price, geometry, pending_reason, ticket))
      {
         ...
         if(UsarMotorAgressivoV121 && UsarEntradasDirecionaisIndependentes)
         {
            ...
            if(RequestPendingCancelV1231(buy_side, ticket, "invalida_" + pending_reason))
      ...
      if(wrong_type)
      {
         if(RequestPendingCancelV1231(buy_side, ticket, "mudanca_tipo_pendente"))
...
void CancelInvalidPendingOrders()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(delete_order)
      {
         ...
         if(RequestPendingCancelV1231(buy_side, ticket,
...
void CancelAllPendingOrders()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      bool buy_side = IsBuyPendingType(type);
      if(RequestPendingCancelV1231(buy_side, ticket, "cancelar_todas_pendentes"))
         LogMessage("cancelamento de pendentes.");
