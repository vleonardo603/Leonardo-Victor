...
#property description "IA FUNDED GOLD V1.2.3.4 - saida com MFE monetario por ticket, SL real confirmado no servidor, teto de perda WEAK e preservacao de vencedoras (GOOD/RUNNER)."
...
// ============================ V1.2.3.4 EXIT MFE SERVER SL FIX ============================
// Estado de protecao de saida ISOLADO POR TICKET. Nenhuma destas variaveis altera
// sinal, entrada, lote, pendentes ou risco; elas apenas registram MFE monetario,
// direitos de Break-even/Lock e a confirmacao do SL real no servidor.
struct ExitTicketStateV1234
{
   ...
   bool     beConfirmed;        // BE confirmado no servidor (POSITION_SL lido)
   bool     lockConfirmed;      // Lock confirmado no servidor (POSITION_SL lido)
...
#define EXIT_TICKET_STATE_MAX_V1234 64
ExitTicketStateV1234 g_exitTicketStatesV1234[EXIT_TICKET_STATE_MAX_V1234];
int g_exitTicketStateCountV1234 = 0;
bool g_exitCloseSentThisTickV1234 = false;
int g_exitWeakClosesV1234 = 0;
int g_exitWeakCloseFailsV1234 = 0;
int g_exitBeConfirmedV1234 = 0;
int g_exitLockConfirmedV1234 = 0;
int g_exitSlNotConfirmedV1234 = 0;
int g_exitMonotonicBlocksV1234 = 0;
int g_exitGoodBlocksV1234 = 0;
int g_exitRunnerBlocksV1234 = 0;
int g_exitGivebackClosesV1234 = 0;
// ========================== FIM ESTADO V1.2.3.4 EXIT PROTECTION ==========================
...
// V1.2.3.4 EXIT MFE SERVER SL FIX - autoridades de protecao de saida por ticket
double PriceDistanceToUSDV1234(double price_distance, double volume);
int    FindExitTicketStateV1234(ulong ticket);
void   RemoveExitTicketStateV1234(int index, string reason);
void   SyncExitTicketStatesV1234();
double ProtectionDesiredSLV1234(int direction, double open_price, double volume, double protect_usd);
bool   ServerSLSatisfiesLevelV1234(int direction, double server_sl, double desired_sl, double tolerance);
bool   RequestProtectionSLV1234(int index, bool lock_tier);
bool   CloseSingleTicketV1234(int index, string tag, string reason);
void   ManageExitProtectionV1234();
bool   BlockGenericBasketExitV1234(string trigger, double basket_profit);
...
int OnInit()
{
   ...
   // V1.2.3.4 EXIT MFE SERVER SL FIX: configuracao ativa da protecao de saida
   LogMessage("[EXIT_ONLY_AUDIT] evento=config enable=" + (ExitProtection_Enable ? "1" : "0") +
              " weak_max_loss=" + DoubleToString(ExitProtection_WeakMaxLossUSD, 2) +
              " weak_max_mfe=" + DoubleToString(ExitProtection_WeakMaxMFEUSD, 2) +
              ...
              " good_faixa=" + DoubleToString(ExitProtection_GoodProfitMinUSD, 2) + ".." + DoubleToString(ExitProtection_GoodProfitMaxUSD, 2) +
...
void OnDeinit(const int reason)
{
   ...
   // V1.2.3.4 EXIT MFE SERVER SL FIX: resumo da protecao de saida por ticket
   LogMessage("[EXIT_ONLY_AUDIT] evento=resumo_deinit weak_closes=" + IntegerToString(g_exitWeakClosesV1234) +
              " weak_close_fails=" + IntegerToString(g_exitWeakCloseFailsV1234) +
              " be_confirmados=" + IntegerToString(g_exitBeConfirmedV1234) +
              " lock_confirmados=" + IntegerToString(g_exitLockConfirmedV1234) +
              " sl_nao_confirmados=" + IntegerToString(g_exitSlNotConfirmedV1234) +
              " bloqueios_monotonicos=" + IntegerToString(g_exitMonotonicBlocksV1234) +
              " good_preservadas=" + IntegerToString(g_exitGoodBlocksV1234) +
              " runner_preservadas=" + IntegerToString(g_exitRunnerBlocksV1234) +
              " giveback_closes=" + IntegerToString(g_exitGivebackClosesV1234) +
              " estados_ativos=" + IntegerToString(g_exitTicketStateCountV1234));
   PersistBasketState();
   if(g_atrHandle != INVALID_HANDLE)
   {
...
void OnTick()
{
   ...
   // V1.2.3.4 EXIT MFE SERVER SL FIX: MFE monetario por ticket, teto WEAK,
   // Break-even/Lock por MFE com SL real confirmado no servidor. Roda depois
   // do risco diario e antes das demais rotinas de saida.
   ManageExitProtectionV1234();
...
void ManagePriceBasedBasketClosing()
{
   ...
   if(touched)
   {
      if(TryExtractWeakSideV123("PRICE_LINE"))
         return;
      // V1.2.3.4 EXIT MFE SERVER SL FIX: saida generica de lucro pequeno nao
      // pode matar GOOD (+5..+10) nem RUNNER; vencedoras ficam protegidas por
      // SL real e a cesta permanece elegivel aos alvos superiores.
      if(BlockGenericBasketExitV1234("PRICE_LINE", profit))
         return;
...
bool GerenciarProtecaoLucroCestaMFE()
{
   ...
   if(g_PicoLucroCestaUSD >= PicoMinimoLucroCestaUSD &&
      ...
      lucroAtualCesta > 0.0)
   {
      // V1.2.3.4 EXIT MFE SERVER SL FIX: o lock da cesta e uma saida generica
      // de lucro pequeno (fecha tudo com lucro atual <= LucroProtegidoAposPicoUSD).
      // Nao pode matar GOOD/RUNNER; o fechamento forte (pico), os alvos da
      // cesta, o DD e o loss diario permanecem intactos.
      if(BlockGenericBasketExitV1234("BASKET_MFE_LOCK", lucroAtualCesta))
         return false;
...
// ============================ V1.2.3.4 EXIT MFE SERVER SL FIX ============================
// Autoridade de protecao de saida POR TICKET. Escopo restrito a: MFE monetario
// por ticket, teto de perda WEAK, Break-even/Lock por MFE, confirmacao do SL
// real no servidor, preservacao de GOOD/RUNNER e fallback de devolucao extrema.
...
int FindExitTicketStateV1234(ulong ticket)
{
   for(int i = 0; i < g_exitTicketStateCountV1234; i++)
   {
      if(g_exitTicketStatesV1234[i].ticket == ticket)
         return i;
   }
   return -1;
}
...
void RemoveExitTicketStateV1234(int index, string reason)
{
   if(index < 0 || index >= g_exitTicketStateCountV1234)
      return;
   LogMessage("[EXIT_ONLY_AUDIT] evento=estado_ticket_removido ticket=" +
              IntegerToString((int)g_exitTicketStatesV1234[index].ticket) +
              " mfe_final=" + DoubleToString(g_exitTicketStatesV1234[index].peakMfeUsd, 2) +
              " lucro_final_visto=" + DoubleToString(g_exitTicketStatesV1234[index].lastProfitUsd, 2) +
              " be_conf=" + (g_exitTicketStatesV1234[index].beConfirmed ? "1" : "0") +
              " lock_conf=" + (g_exitTicketStatesV1234[index].lockConfirmed ? "1" : "0") +
              " motivo=" + reason);
   int last = g_exitTicketStateCountV1234 - 1;
   if(index != last)
      g_exitTicketStatesV1234[index] = g_exitTicketStatesV1234[last];
   g_exitTicketStateCountV1234 = last;
}

// Sincroniza o estado por ticket com o servidor: cria estado para posicao nova,
// atualiza o MFE monotonico (peak = MathMax(peak, lucro_atual)) e limpa estado
// SOMENTE quando o ticket ja nao existe mais no servidor (confirmacao real de
// encerramento). BUY e SELL nunca compartilham estado: a chave e o ticket.
void SyncExitTicketStatesV1234()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      double profit = GetSelectedPositionProfitWithCosts();
      int idx = FindExitTicketStateV1234(ticket);
      if(idx < 0)
      {
         if(g_exitTicketStateCountV1234 >= EXIT_TICKET_STATE_MAX_V1234)
         {
            LogMessage("[EXIT_ONLY_AUDIT] evento=tabela_estado_cheia ticket=" + IntegerToString((int)ticket));
            continue;
         }
         idx = g_exitTicketStateCountV1234;
         g_exitTicketStateCountV1234++;
         g_exitTicketStatesV1234[idx].ticket = ticket;
         g_exitTicketStatesV1234[idx].symbol = PositionGetString(POSITION_SYMBOL);
         g_exitTicketStatesV1234[idx].magic = (long)PositionGetInteger(POSITION_MAGIC);
         g_exitTicketStatesV1234[idx].direction = (int)PositionGetInteger(POSITION_TYPE);
         g_exitTicketStatesV1234[idx].volume = PositionGetDouble(POSITION_VOLUME);
         g_exitTicketStatesV1234[idx].openPrice = PositionGetDouble(POSITION_PRICE_OPEN);
         g_exitTicketStatesV1234[idx].peakMfeUsd = MathMax(0.0, profit);
         g_exitTicketStatesV1234[idx].lastProfitUsd = profit;
         g_exitTicketStatesV1234[idx].beRight = false;
         g_exitTicketStatesV1234[idx].lockRight = false;
         g_exitTicketStatesV1234[idx].beConfirmed = false;
         g_exitTicketStatesV1234[idx].lockConfirmed = false;
         g_exitTicketStatesV1234[idx].lastRequestedSL = 0.0;
         g_exitTicketStatesV1234[idx].confirmedServerSL = 0.0;
         g_exitTicketStatesV1234[idx].lastModifyAttempt = 0;
         g_exitTicketStatesV1234[idx].modifyAttempts = 0;
         g_exitTicketStatesV1234[idx].lastCloseAttempt = 0;
         g_exitTicketStatesV1234[idx].closeAttempts = 0;
         g_exitTicketStatesV1234[idx].lastLoggedMfeUsd = MathMax(0.0, profit);
         ...
         if(existing_sl > 0.0)
         {
            g_exitTicketStatesV1234[idx].confirmedServerSL = existing_sl;
            int dir = g_exitTicketStatesV1234[idx].direction;
            double signed_dist = (dir == (int)POSITION_TYPE_BUY
                                  ? existing_sl - g_exitTicketStatesV1234[idx].openPrice
                                  : g_exitTicketStatesV1234[idx].openPrice - existing_sl);
            double sl_usd = PriceDistanceToUSDV1234(signed_dist, g_exitTicketStatesV1234[idx].volume);
            if(sl_usd >= ExitProtection_BEProfitUSD - 0.01)
            {
               g_exitTicketStatesV1234[idx].beRight = true;
               g_exitTicketStatesV1234[idx].beConfirmed = true;
            }
            if(sl_usd >= ExitProtection_LockProfitUSD - 0.01)
            {
               g_exitTicketStatesV1234[idx].lockRight = true;
               g_exitTicketStatesV1234[idx].lockConfirmed = true;
            }
         }
         LogMessage("[EXIT_ONLY_AUDIT] evento=estado_ticket_criado ticket=" + IntegerToString((int)ticket) +
                    " direcao=" + (g_exitTicketStatesV1234[idx].direction == (int)POSITION_TYPE_BUY ? "BUY" : "SELL") +
                    " volume=" + DoubleToString(g_exitTicketStatesV1234[idx].volume, 2) +
                    " abertura=" + DoubleToString(g_exitTicketStatesV1234[idx].openPrice, _Digits) +
                    " lucro=" + DoubleToString(profit, 2) +
                    " mfe_inicial=" + DoubleToString(g_exitTicketStatesV1234[idx].peakMfeUsd, 2) +
      ...
      g_exitTicketStatesV1234[idx].lastProfitUsd = profit;
      g_exitTicketStatesV1234[idx].volume = PositionGetDouble(POSITION_VOLUME);
      if(profit > g_exitTicketStatesV1234[idx].peakMfeUsd)
      {
         g_exitTicketStatesV1234[idx].peakMfeUsd = profit;
         if(g_exitTicketStatesV1234[idx].peakMfeUsd - g_exitTicketStatesV1234[idx].lastLoggedMfeUsd >= 1.0)
         {
            g_exitTicketStatesV1234[idx].lastLoggedMfeUsd = g_exitTicketStatesV1234[idx].peakMfeUsd;
            LogMessage("[MFE_UPDATED] ticket=" + IntegerToString((int)ticket) +
                       " direcao=" + (g_exitTicketStatesV1234[idx].direction == (int)POSITION_TYPE_BUY ? "BUY" : "SELL") +
                       " lucro_atual=" + DoubleToString(profit, 2) +
                       " mfe=" + DoubleToString(g_exitTicketStatesV1234[idx].peakMfeUsd, 2) +
                       " volume=" + DoubleToString(g_exitTicketStatesV1234[idx].volume, 2));
         }
      }
   }
   ...
   for(int s = g_exitTicketStateCountV1234 - 1; s >= 0; s--)
   {
      if(!PositionSelectByTicket(g_exitTicketStatesV1234[s].ticket))
         RemoveExitTicketStateV1234(s, "ticket_confirmado_encerrado_no_servidor");
   }
}
...
// Fluxo obrigatorio: calcular -> normalizar -> preservar TP -> modificar pelo
// ticket -> ler retcode -> re-selecionar pelo ticket -> ler POSITION_SL ->
// comparar com tolerancia de tick. Flag interna SOMENTE apos a confirmacao.
bool RequestProtectionSLV1234(int index, bool lock_tier)
{
   if(index < 0 || index >= g_exitTicketStateCountV1234)
      return false;

   ulong ticket = g_exitTicketStatesV1234[index].ticket;
   datetime now = TimeCurrent();
   ...
   if(g_exitTicketStatesV1234[index].modifyAttempts >= MathMax(1, ExitProtection_MaxModifyAttempts))
      cooldown *= 10;
   if(g_exitTicketStatesV1234[index].lastModifyAttempt > 0 &&
      (int)(now - g_exitTicketStatesV1234[index].lastModifyAttempt) < cooldown)
      return false;
   ...
   double desired_sl = ProtectionDesiredSLV1234(direction, open_price, volume, protect_usd);
   if(desired_sl <= 0.0)
      return false;

   double tick_size = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE);
   double tolerance = MathMax(tick_size, _Point) * MathMax(1.0, ToleranciaConfirmacaoSLTicks);

   // SL ja existente igual ou melhor que o tier: confirmar sem nova modificacao.
   if(ServerSLSatisfiesLevelV1234(direction, server_sl, desired_sl, tolerance))
   {
      g_exitTicketStatesV1234[index].confirmedServerSL = server_sl;
      g_exitTicketStatesV1234[index].modifyAttempts = 0;
      if(lock_tier)
      {
         if(!g_exitTicketStatesV1234[index].lockConfirmed)
         {
            g_exitTicketStatesV1234[index].lockConfirmed = true;
            g_exitTicketStatesV1234[index].beConfirmed = true;
            g_exitLockConfirmedV1234++;
            LogMessage("[SL_SERVER_CONFIRMED] ticket=" + IntegerToString((int)ticket) +
                       " tier=LOCK origem=sl_existente_ja_melhor sl_servidor=" + DoubleToString(server_sl, _Digits));
         }
      }
      else if(!g_exitTicketStatesV1234[index].beConfirmed)
      {
         g_exitTicketStatesV1234[index].beConfirmed = true;
         g_exitBeConfirmedV1234++;
         LogMessage("[SL_SERVER_CONFIRMED] ticket=" + IntegerToString((int)ticket) +
                    " tier=BE origem=sl_existente_ja_melhor sl_servidor=" + DoubleToString(server_sl, _Digits));
      }
      return true;
   }
   ...
   if(server_sl > 0.0)
   {
      ...
      if(MathAbs(clamped - server_sl) <= tolerance)
      {
         ...
         LogMessage("[SL_MONOTONIC_BLOCK] ticket=" + IntegerToString((int)ticket) +
   ...
   if(!IsSLValid(ptype, desired_sl))
   {
      ...
      g_exitTicketStatesV1234[index].lastModifyAttempt = now;
      g_exitTicketStatesV1234[index].modifyAttempts++;
      g_exitSlNotConfirmedV1234++;
      LogThrottledV1232("v1234_sl_invalid_" + IntegerToString((int)ticket),
                        "[SL_SERVER_NOT_CONFIRMED] ticket=" + IntegerToString((int)ticket) +
                        " tier=" + (lock_tier ? "LOCK" : "BE") +
                        ...
                        " tentativas=" + IntegerToString(g_exitTicketStatesV1234[index].modifyAttempts), 10);
      return false;
   }

   g_exitTicketStatesV1234[index].lastModifyAttempt = now;
   g_exitTicketStatesV1234[index].modifyAttempts++;
   g_exitTicketStatesV1234[index].lastRequestedSL = desired_sl;
   ...
   string tier_reason = (lock_tier ? "lock_mfe_v1234" : "breakeven_mfe_v1234");
   ...
   if(confirmed)
   {
      ...
      if(!ServerSLSatisfiesLevelV1234(direction, confirmed_sl, desired_sl, tolerance))
      {
         g_exitSlNotConfirmedV1234++;
         LogMessage("[SL_SERVER_NOT_CONFIRMED] ticket=" + IntegerToString((int)ticket) +
                    " tier=" + (lock_tier ? "LOCK" : "BE") +
                    " motivo=releitura_divergente esperado=" + DoubleToString(desired_sl, _Digits) +
                    " servidor=" + DoubleToString(confirmed_sl, _Digits));
         return false;
      }
      g_exitTicketStatesV1234[index].confirmedServerSL = confirmed_sl;
      g_exitTicketStatesV1234[index].modifyAttempts = 0;
      if(lock_tier)
      {
         g_exitTicketStatesV1234[index].lockConfirmed = true;
...
// (pelo SyncExitTicketStatesV1234 do proximo ciclo, que reconfirma no servidor).
bool CloseSingleTicketV1234(int index, string tag, string reason)
{
   if(index < 0 || index >= g_exitTicketStateCountV1234)
      return false;
   ...
   if(g_exitTicketStatesV1234[index].closeAttempts >= MathMax(1, ExitProtection_MaxModifyAttempts))
      cooldown = base_cooldown * 10; // backoff: mercado fechado, rejeicao, iliquidez
   if(g_exitTicketStatesV1234[index].lastCloseAttempt > 0 &&
      (int)(now - g_exitTicketStatesV1234[index].lastCloseAttempt) < cooldown)
      return false;
   ...
   LogMessage(tag + "_REQUEST] ticket=" + IntegerToString((int)ticket) +
              " simbolo=" + _Symbol +
              ...
              " lucro=" + DoubleToString(profit, 2) +
              " mfe=" + DoubleToString(g_exitTicketStatesV1234[index].peakMfeUsd, 2) +
              " motivo=" + reason +
              " obs=slippage_gap_ou_spread_podem_variar_o_valor_final");

   g_exitTicketStatesV1234[index].lastCloseAttempt = now;
   g_exitTicketStatesV1234[index].closeAttempts++;
   ...
   if(!sent || !IsTradeRetcodeSuccessV122(retcode))
   {
      g_exitWeakCloseFailsV1234++;
      LogMessage(tag + "_FAILED] ticket=" + IntegerToString((int)ticket) +
                 " retcode=" + IntegerToString((int)retcode) +
                 " desc=" + trade.ResultRetcodeDescription() +
                 " tentativas=" + IntegerToString(g_exitTicketStatesV1234[index].closeAttempts));
      return false;
   }
   ...
   if(PositionSelectByTicket(ticket))
   {
      // Envio aceito mas ticket ainda visivel: aguardar reconciliacao.
      LogMessage(tag + "_FAILED] ticket=" + IntegerToString((int)ticket) +
                 " motivo=aguardando_confirmacao_servidor retcode=" + IntegerToString((int)retcode));
      return false;
   }

   LogMessage(tag + "_CONFIRMED] ticket=" + IntegerToString((int)ticket) +
              " lucro_no_pedido=" + DoubleToString(profit, 2) +
              " retcode=" + IntegerToString((int)retcode) +
              " posicao_inexistente_no_servidor=1");
   RemoveExitTicketStateV1234(index, "fechamento_confirmado_" + reason);
   return true;
}

// AUTORIDADE POR TICKET (ordem interna): 1) teto de perda WEAK; 2) direitos de
// BE/Lock por MFE (permanentes, imunes a pullback); 3) SL real do tier mais
// alto pendente; 4) fallback de devolucao extrema sem SL confirmado.
void ManageExitProtectionV1234()
{
   ...
   // MFE continua sendo atualizado mesmo durante fechamentos (somente leitura).
   SyncExitTicketStatesV1234();

   if(g_exitTicketStateCountV1234 <= 0)
      return;
   ...
   // Classificacao existente por lado (criterios INALTERADOS).
   string buy_class = ClassifySideV123(POSITION_TYPE_BUY);
   string sell_class = ClassifySideV123(POSITION_TYPE_SELL);

   for(int idx = g_exitTicketStateCountV1234 - 1; idx >= 0; idx--)
   {
      ulong ticket = g_exitTicketStatesV1234[idx].ticket;
      if(!PositionSelectByTicket(ticket))
      ...
      double peak = g_exitTicketStatesV1234[idx].peakMfeUsd;

      // (1) TETO DE PERDA WEAK: classe existente WEAK + pouco ou nenhum MFE.
      if(side_class == "WEAK" &&
         profit <= -MathAbs(ExitProtection_WeakMaxLossUSD) &&
         peak <= ExitProtection_WeakMaxMFEUSD &&
         !g_exitCloseSentThisTickV1234)
      {
         LogMessage("[WEAK_MAX_LOSS_TRIGGER] ticket=" + IntegerToString((int)ticket) +
                    " classe=" + side_class +
                    " lucro=" + DoubleToString(profit, 2) +
                    " limite=" + DoubleToString(-MathAbs(ExitProtection_WeakMaxLossUSD), 2) +
                    " mfe=" + DoubleToString(peak, 2) +
                    " mfe_max_permitido=" + DoubleToString(ExitProtection_WeakMaxMFEUSD, 2));
         LogMessage("[EXIT_AUTHORITY_DECISION] ticket=" + IntegerToString((int)ticket) +
                    " classe=" + side_class + " decisao=CLOSE autoridade=WEAK_MAX_LOSS");
         if(CloseSingleTicketV1234(idx, "[WEAK_CLOSE", "weak_max_loss"))
            g_exitWeakClosesV1234++;
      ...
      if(!g_exitTicketStatesV1234[idx].lockRight && peak >= ExitProtection_LockTriggerMFEUSD)
      {
         g_exitTicketStatesV1234[idx].lockRight = true;
         g_exitTicketStatesV1234[idx].beRight = true;
         LogMessage("[LOCK_MFE_TRIGGER] ticket=" + IntegerToString((int)ticket) +
                    " mfe=" + DoubleToString(peak, 2) +
         ...
         if(profit < ExitProtection_LockTriggerMFEUSD)
            LogMessage("[PROFIT_MEMORY_PROTECTION] ticket=" + IntegerToString((int)ticket) +
                       " mfe=" + DoubleToString(peak, 2) +
                       " lucro_atual=" + DoubleToString(profit, 2) +
                       " direito=LOCK mantido_apos_pullback=1");
      }
      if(!g_exitTicketStatesV1234[idx].beRight && peak >= ExitProtection_BETriggerMFEUSD)
      {
         g_exitTicketStatesV1234[idx].beRight = true;
         LogMessage("[BE_MFE_TRIGGER] ticket=" + IntegerToString((int)ticket) +
                    " mfe=" + DoubleToString(peak, 2) +
      ...
      if(g_exitTicketStatesV1234[idx].lockRight && !g_exitTicketStatesV1234[idx].lockConfirmed)
         RequestProtectionSLV1234(idx, true);
      else if(g_exitTicketStatesV1234[idx].beRight && !g_exitTicketStatesV1234[idx].beConfirmed)
         RequestProtectionSLV1234(idx, false);

      // (4) DEVOLUCAO EXTREMA: somente sem NENHUM SL protetor confirmado,
      // com tentativas esgotadas e devolucao perigosa do lucro.
      if(g_exitTicketStatesV1234[idx].beRight &&
         !g_exitTicketStatesV1234[idx].beConfirmed &&
         !g_exitTicketStatesV1234[idx].lockConfirmed &&
         ExitProtection_EmergencyGivebackUSD > 0.0 &&
         g_exitTicketStatesV1234[idx].modifyAttempts >= MathMax(1, ExitProtection_MaxModifyAttempts) &&
         profit <= g_exitTicketStatesV1234[idx].peakMfeUsd - MathAbs(ExitProtection_EmergencyGivebackUSD) &&
         !g_exitCloseSentThisTickV1234)
      {
         LogMessage("[EMERGENCY_GIVEBACK_TRIGGER] ticket=" + IntegerToString((int)ticket) +
                    " mfe=" + DoubleToString(g_exitTicketStatesV1234[idx].peakMfeUsd, 2) +
                    " lucro=" + DoubleToString(profit, 2) +
                    " devolucao=" + DoubleToString(g_exitTicketStatesV1234[idx].peakMfeUsd - profit, 2) +
                    " limite=" + DoubleToString(MathAbs(ExitProtection_EmergencyGivebackUSD), 2) +
                    " tentativas_sl=" + IntegerToString(g_exitTicketStatesV1234[idx].modifyAttempts) +
                    " sl_confirmado=0");
         LogMessage("[EXIT_AUTHORITY_DECISION] ticket=" + IntegerToString((int)ticket) +
                    " classe=" + side_class + " decisao=CLOSE autoridade=EMERGENCY_GIVEBACK");
         if(CloseSingleTicketV1234(idx, "[GIVEBACK_CLOSE", "emergency_giveback"))
            g_exitGivebackClosesV1234++;
      }
   }
}

// Guard das saidas GENERICAS de cesta (linha dinamica >= US$5 e lock da cesta
// <= US$7): nao podem matar GOOD (+5..+10) nem RUNNER/PROTECTED nem ticket com
// direito de Lock por MFE. NAO bloqueia: alvo da cesta, fechamento forte por
// pico, extracao do lado fraco, DD, loss diario, emergencia ou fechamento manual.
bool BlockGenericBasketExitV1234(string trigger, double basket_profit)
{
   if(!ExitProtection_Enable)
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      int direction = (int)PositionGetInteger(POSITION_TYPE);
      string side_class = (direction == (int)POSITION_TYPE_BUY ? buy_class : sell_class);
      int state_idx = FindExitTicketStateV1234(ticket);
      double peak = (state_idx >= 0 ? g_exitTicketStatesV1234[state_idx].peakMfeUsd : MathMax(0.0, profit));
      bool lock_right = (state_idx >= 0 && g_exitTicketStatesV1234[state_idx].lockRight);

      // RUNNER/PROTECTED (classes vencedoras existentes) ou ticket com Lock por
      // MFE: imune a saida generica; so sai por SL real, trailing, alvo, risco.
      if(side_class == "RUNNER" || side_class == "PROTECTED" || lock_right)
      {
         g_exitRunnerBlocksV1234++;
         LogThrottledV1232("v1234_runner_block_" + trigger,
                           "[RUNNER_GENERIC_EXIT_BLOCKED] trigger=" + trigger +
                           " ticket=" + IntegerToString((int)ticket) +
                           " classe=" + side_class +
                           " lock_mfe=" + (lock_right ? "1" : "0") +
                           " lucro=" + DoubleToString(profit, 2) +
                           " mfe=" + DoubleToString(peak, 2) +
                           " cesta=" + DoubleToString(basket_profit, 2) +
                           " decisao=HOLD", 10);
         LogThrottledV1232("v1234_authority_runner_" + trigger,
                           "[EXIT_AUTHORITY_DECISION] trigger=" + trigger +
                           " decisao=HOLD autoridade=RUNNER_PRESERVATION", 10);
         return true;
      }

      // GOOD: lucro atual na faixa +5..+10; precisa respirar para virar RUNNER.
      if(profit >= ExitProtection_GoodProfitMinUSD && profit <= ExitProtection_GoodProfitMaxUSD)
      {
         g_exitGoodBlocksV1234++;
         LogThrottledV1232("v1234_good_block_" + trigger,
                           "[GOOD_SMALL_PROFIT_EXIT_BLOCKED] trigger=" + trigger +
                           " ticket=" + IntegerToString((int)ticket) +
                           " classe=" + side_class +
                           " lucro=" + DoubleToString(profit, 2) +
                           " mfe=" + DoubleToString(peak, 2) +
                           " cesta=" + DoubleToString(basket_profit, 2) +
                           " motivo_original=" + trigger +
                           " decisao=HOLD", 10);
         LogThrottledV1232("v1234_authority_good_" + trigger,
                           "[EXIT_AUTHORITY_DECISION] trigger=" + trigger +
                           " decisao=HOLD autoridade=GOOD_PRESERVATION", 10);
         return true;
      }
   }
...
// ========================== FIM V1.2.3.4 EXIT MFE SERVER SL FIX ==========================
...
void UpdatePanel()
{
   ...
   string panel =
      EA_NAME + "\n" +
      "Versao: V1.2.3.4 EXIT MFE SERVER SL FIXED\n" +
      "Simbolo: " + _Symbol + "\n" +
...
bool ModifyPositionSL(ulong ticket, double new_sl, double current_tp, string reason)
{
   ...
   LogMessage("[SL_MODIFY_REQUEST] ticket=" + IntegerToString((int)ticket) +
              " direcao=" + (type == POSITION_TYPE_BUY ? "BUY" : "SELL") +
              " sl_atual=" + DoubleToString(existing_sl, _Digits) +
              " sl_solicitado=" + DoubleToString(requested_sl, _Digits) +
   ...
   uint retcode = trade.ResultRetcode();
   LogMessage("[SL_MODIFY_RETCODE] ticket=" + IntegerToString((int)ticket) +
              " retcode=" + IntegerToString((int)retcode) +
              " desc=" + trade.ResultRetcodeDescription() +
